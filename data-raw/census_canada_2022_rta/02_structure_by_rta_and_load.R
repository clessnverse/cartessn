...
df_raw <- read.csv("data-raw/data/census_canada_2022_rta/98-401-X2021013_English_CSV_data.csv", 
                 fileEncoding = "latin1") |>
  filter(GEO_LEVEL == "Forward sortation area")
...
census_canada_2022_rta <- df_raw |> 
  ...
  ## add the gender df
  rbind(., df_gender) |> 
  arrange(rta, variable, category) |>
  group_by(
    rta, variable, category
  ) |> 
